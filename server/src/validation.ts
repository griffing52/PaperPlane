...
export const CreateFlightSchema = z.object({
  pilotId: z.string().uuid('Invalid pilot ID format'),
  uploadId: z.string().uuid('Invalid upload ID format').nullable().optional(),
  departureAirfield: z.string().min(1, 'Departure airfield is required'),
  tailNumber: z.string().min(1, 'Tail number is required'),
  depDate: z.string().datetime('Invalid date format, expected ISO 8601 datetime'),
  hours: z.number().positive('Hours must be a positive number').or(
    z.string().refine(
...
export const FlightsQuerySchema = z.object({
  pilotId: z.string().uuid('Invalid pilot ID format').optional(),
});
